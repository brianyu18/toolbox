...
| THINK | `thinker` skill |
| PLAN | `planner` skill |
| PLAN-REVIEW (gate) | **Conditional cross-model gate — fires when tier is `complex`, OR the plan touches auth, schema/migrations, payments/money, user-data deletion, or external-facing APIs.** LEAD invokes the user skill `plan-review` (Skill tool) on the locked plan file. Requires Codex CLI logged in (`codex login status`). **Complex tier: the gate is MANDATORY — if Codex is unavailable, do NOT silently proceed; surface it to the user as a blocking question (fix login / explicitly waive / abort).** For non-complex risk-trigger plans only, unavailability may log `[LEAD#] WARN plan-review skipped: codex unavailable` and proceed. LEAD is the arbiter (never delegate arbitration to a worker). `VERDICT: APPROVED` → BUILD. Deadlock after 5 rounds → funnel the unresolved findings to the user like any blocking question. Ledger: `PLAN-REVIEW-LOG.md` next to the plan. |
| BUILD | LEAD reads `state/plan-partitions.md` (PLANNER's wave-grouped partitions). For each wave with `parallel_safe: true`, LEAD dispatches N `builder` workers in parallel via Task tool (one Task message, multiple tool blocks). For sequential waves, dispatches in order. LEAD merges/applies/commits diffs serially after each wave. Failed partitions: retry once; on second failure, escalate. (See `dispatch-recipes.md` for brief composition.) **Codex lane (optional):** a partition PLANNER marked `contract_isolated: true` (own directory, frozen interface, no mid-wave coordination) MAY be routed through the user skill `codex-builder` instead of a `builder` worker when Codex CLI is authed — moves build tokens to the ChatGPT meter. Never route UI partitions to Codex (presto-first: design direction is Claude-side only). |
