...
| THINK | `thinker` skill |
| PLAN | `planner` skill |
| PLAN-REVIEW (gate) | **Conditional cross-model gate — fires when tier is `complex`, OR the plan touches auth, schema/migrations, payments/money, user-data deletion, or external-facing APIs.** LEAD invokes the user skill `plan-review` (Skill tool) on the locked plan file. Requires Codex CLI logged in (`codex login status`); if unavailable, log `[LEAD#] WARN plan-review skipped: codex unavailable` and proceed. LEAD is the arbiter (never delegate arbitration to a worker). `VERDICT: APPROVED` → BUILD. Deadlock after 5 rounds → funnel the unresolved findings to the user like any blocking question. Ledger: `PLAN-REVIEW-LOG.md` next to the plan. |
