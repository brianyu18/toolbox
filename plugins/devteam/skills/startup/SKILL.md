...
- **PLAN-REVIEW** (gate; fires when the contract tier is complex OR the plan touches auth, schema/migrations, payments/money, user-data deletion, or external-facing APIs) — MAIN THREAD: invoke the user skill `plan-review` (Skill tool) on `state/plan-partitions.md`'s source plan. STARTUP is the arbiter; log each round `[STARTUP#] INFO PLAN-REVIEW round N: <verdict>`. `VERDICT: APPROVED` → BUILD. Deadlock after 5 rounds → 🔴 escalation gate (unresolved cross-model disagreement is a blocker council-lite cannot resolve). Codex CLI unavailable → log a warning and proceed unGated.
