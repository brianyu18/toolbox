...
- **PLAN-REVIEW** (gate; fires when the contract tier is complex OR the plan touches auth, schema/migrations, payments/money, user-data deletion, or external-facing APIs) — MAIN THREAD: invoke the user skill `plan-review` (Skill tool) on `state/plan-partitions.md`'s source plan. STARTUP is the arbiter; log each round `[STARTUP#] INFO PLAN-REVIEW round N: <verdict>`. `VERDICT: APPROVED` → BUILD. Deadlock after 5 rounds → 🔴 escalation gate (unresolved cross-model disagreement is a blocker council-lite cannot resolve). Codex CLI unavailable on a COMPLEX-tier contract → 🔴 escalation gate (the mandatory cross-model review cannot run; user must fix login, waive, or abort); on non-complex risk-trigger plans, log a warning and proceed.
- **BUILD** — per wave (respect `fanout`): dispatch `builder` for non-UI partitions and `frontend-specialist` for UI partitions, in parallel. Partitions marked `contract_isolated: true` MAY route through the user skill `codex-builder` (Codex lane, ChatGPT meter) when Codex is authed — never UI partitions (presto-first). UI briefs include the DESIGN artifact paths + the presto taste-skill paths. Merge/commit diffs serially after each wave.
